...
# initialize file with numerical results
with open("results/results.txt", "w", encoding="utf-8") as f:
    ...
    for experiment in experiments:
        ...
        # Check initial and final positions (0 heading added to get correct dimension)
        x_init: np.ndarray = np.array(data["x_init"]).T
        x_goal: np.ndarray = np.array(data["x_goal"]).T
        x_init = (
            np.vstack([x_init, np.zeros([1, m])]) if x_init.shape[0] == 2 else x_init
        )
        x_goal = (
            np.vstack([x_goal, np.zeros([1, m])]) if x_goal.shape[0] == 2 else x_goal
        )
        ...
        if not geometry.check_positions(
            ...
            raise ValueError(
                "Goal positions are not admissible for collision avoidance."
            )
        ...
        if "braid" in data:
            ...
            # n_generators_braid = braid.shape[0]  # number of generators in the word
            # n_generators = n_generators_braid
            # w_target_full = invariants.braid2windings(braid, m)  --> to implement
        if "grids" in data:
            grids = np.array(data["grids"])
            n_generators_grid = grids.shape[0]  # number of generators in specification
        ...
        for controller in controllers:
            ...
            for i in range(m):
                ...
                M[i].x_opt = np.tile(
                    M[i].x, (K + 1, 1)
                )  # initialize with constant state
            ...
            delta_tau: float = (
                2
                / (n_generators * np.pi)
                * np.arcsin(np.linalg.norm(u_max) * dt / d_min)
            )
            ...
            if controller == "centralized":
                ...
                while goal_reached is False and step < n_time:
                    ...
                    alpha_w_k = (
                        ...
                        * (
                        ...
                        )  # sigmoid-based weight (decreasing with tau to track goal)
                ...
                fig_w_target.savefig(
                    os.path.join(output_dir, "c_w_target.png"), dpi=900
                )
                ...
                fig_w_target.savefig(
                    os.path.join(output_dir, "d_w_target.png"), dpi=900
                )
                ...
                while goal_reached is False and step < n_time:
                    ...
                    for i in range(m):
                        x_opt, u_opt, cost, t_sol = grid_controller(
                            ...
                            u_prev=M[i].v[
                                :2
                            ],  # note: v = u (without multiplication by dt)
                        )
                        M[i].u_opt = u_opt
                        M[i].x_opt = x_opt
                        M[i].cost = cost
