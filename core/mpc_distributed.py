...
class DistributedMPC(MPC):
    ...
    def __init__(self):
        ...
        self.x_pred: list[ca.Opti.parameter]  # agents' predicted states (K+1, n_x, m-1)
    ...
    def _solve(
        ...
        **kwargs,
        ...
        # Parse kwargs
        x_pred_kwarg = kwargs.get("x_pred", None)
        if x_pred_kwarg is None:
            raise ValueError("x_pred must be provided as a kwarg parameter.")
        elif not isinstance(x_pred_kwarg, list) or len(x_pred_kwarg) != self.m - 1:
            raise ValueError(
                f"x_pred kwarg must be a list of length {self.m - 1}, but got "
